...
amis <- function(prevalence_map, transmission_model, n_params, nsamples, IO_file_id, delta = 5, T = 100, target_ess = 250) {
    ...
    simulated_prevalences <- run_transmission_model(seeds = 1:nsamples, parameters = param[,1], id(1))
    WW <- compute_weight_matrix(prev, simulated_prevalences, delta, first_weight = rep(1, nsamples))
