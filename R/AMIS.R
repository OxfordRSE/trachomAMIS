...
#'
#' @param prevalence_map An L x M matrix containing samples from the prevalence map, where L is the number of locations and M the number of samples.
#' The location names are inherited from rownames(prevalence_map) if possible.
#' @param transmission_model A function taking a vector of seeds and a matrix of parameter vectors as inputs.
#' @param prior A list containing the functions dprior and rprior (density and RNG).
#' rprior must produce an n by d MATRIX of parameters.
#' parameter names are inherited from the colnames from the output of rprior if possible.
#' @param amis_params A list containing the control parameters for the AMIS algorithm
#' \describe{
#' \item{\code{delta} the smoothing parameter is the RN derivative (usually 0.01).}
...
#' \item{\oode{log} logical indicating if calculations are to be performed on log scale. (TODO)} 
#' \item{\code{T} maximum number of AMIS iterations.} }
#' @param seed Optional seed for the random number generator
#' @return A dataframe containing the sampled parameters, simulation seed, and weight in each location  
#' @export
# SS note: Provide option to supply prevalence_map as an analytical function (lieklihood and data), rather than Monte Carlo samples
